...
public class SalesContractApprovalApi {
    ...
    public PageBaseDto<List<Contract>> approvalList(ContractApprovalListDto contractApprovalListDto) {
        ...
        contractApprovalListDto.setType(new Short("1"));    //  1 - 销售单合同
    ...
    public JSONObject pendingNum() {
        Long companyId = SecurityInfoGetter.getCompanyId(); //  获取companyId
        Long userId = SecurityInfoGetter.getUser().getUserId();    //获取user_id
        int total = contractApprovalService.pendingApprovalNum(userId,companyId,new Short("1"));
    ...
    @PreAuthorize("hasRole('ROLE_SYS_ADMIN') or hasAuthority('sales_contract_approval_operate')")
