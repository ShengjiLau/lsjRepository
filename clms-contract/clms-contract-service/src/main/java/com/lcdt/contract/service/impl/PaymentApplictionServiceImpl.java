...
public class PaymentApplictionServiceImpl implements PaymentApplictionService {
    ...
    public PageInfo<List<PaymentApplication>> paymentApplictionList(PaymentApplicationDto paymentApplicationDto, PageInfo pageInfo) {
        ...
        return new PageInfo(paymentApplicationList);
    }
    ...
    public int addPaymentAppliction(PaymentApplicationDto paymentApplicationDto) {
        ...
        if (row > 0) {
            ...
            if (null != paymentApplicationDto.getPaApprovalList() && !paymentApplicationDto.getPaApprovalList().isEmpty()) {
                ...
                for (PaApproval paApproval : paymentApplicationDto.getPaApprovalList()) {
                    ...
                    if (paApproval.getActionType().shortValue() == 0) {
                        if (paApproval.getSort() == 1) {
                            ...
                            attachment.setPaymentSerialNum(queryApplication.getApplicationSerialNo());
                            ...
                            if (queryApplication.getApplicationType().shortValue() == 1) {
                                eventName = "receivable_approval_publish";
                                attachment.setSaleRecSerialNum(queryApplication.getApplicationSerialNo());
                            }
    ...
    public int confirmPayment(PaymentApplication paymentApplication) {
        ...
        if (row > 0) {
            ...
            ContractAttachment attachment = new ContractAttachment();
            attachment.setPaymentSerialNum(pa.getApplicationSerialNo());
            attachment.setCarrierWebNotifyUrl("");
            String eventName = "payment_confirm";
            if (pa.getApplicationType().shortValue() == 1) {
                eventName = "receivable_confirm";
                attachment.setSaleRecSerialNum(pa.getApplicationSerialNo());
            }
            ContractNotifyEvent planPublishEvent = new ContractNotifyEvent(eventName, attachment, defaultNotifyReceiver, defaultNotifySender);
            producer.sendNotifyEvent(planPublishEvent);
            /**↑发送消息通知结束*/
